...
Despite this approximation, using only about 2\% of all possible pair comparisons, we outperform or match baseline performances that compare all jobs for scheduling.\footnote{Further analysis is presented in the Technical Appendix.}
...
Starting from Problem~1, SASS outperforms the baselines across all noise levels $\eta$ on \emph{linear} datasets, as shown in Table~\ref{tab:linear_dataset_results}.\footnote{For more elaborate results including confidence intervals, see Technical Appendix.} Figure~\ref{fig:nonLinear_pronblem_1_2} shows the results on \emph{nonlinear} datasets, where SASS remains competitive with the other baselines, which tend to be more stable and exhibit lower standard deviation across runs. MSE performs best at noise levels $0$ and $6$ (Figure~\ref{fig:nonLinear_pronblem_1_2}), especially at $0$, a typical prediction scenario, but exhibits high variability across runs.
...
\begin{table}[t]
\caption{SCT and Makespan \% error respect to optimal schedule, across different noise level $\eta$ over \emph{linear} dataset with size $10000$ for Problem~1 and Problem~2,  reporting the mean over 10 runs.}
\centering
...
\end{table}
SPO+, on the other hand, exhibits weak performance both with and without noise, in both \emph{linear} and \emph{nonlinear} settings, as reported in Figure~\ref{fig:nonLinear_pronblem_1_2}. The weak performance of SPO+ in Problem~1 stems from the definition of the problem itself. SPO+ works best when the optimization objective changes smoothly with respect to the predicted order. In contrast, for the SCT problem, even a small error in the predicted ordering can lead to large deviations in the objective, which makes SPO+ less effective.
...
\label{fig:nonLinear_pronblem_1_2}
\end{figure}
...
The dataset spans two months (January and November 2021) and contains approximately 201,000 patient pathways derived from real-time location system (RTLS) events.
...
%\todo[inline]{To add discuss on CP or ALMM solution. Insert our paper regarding handling uncertainty durations}
...
Figure~\ref{fig:Linear_dataset_1_machine_problem} highlights the large variance in MSE, while LTR and SASS show a linear progression across the different noise levels. SASS consistently produces a lower SCT error compared to the baselines and, especially at high noise levels, demonstrates strong stability, as indicated by the lower variance in the results (Figure~\ref{fig:Linear_dataset_1_machine_problem}).
