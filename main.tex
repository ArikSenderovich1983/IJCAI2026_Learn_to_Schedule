...
To validate this positioning, we compare SASS against predict-then-optimize (MSE), learn-to-rank (LTR), and an SPO-based baseline, namely SPO+~\cite{elmachtoub2022smart}.
MSE is unaware of the scheduling objective and only penalizes pointwise prediction error, so improvements in MSE do not necessarily translate to better schedules.
LTR treats scheduling as a ranking problem and learns correct orderings, but its loss is generic and does not weight mistakes by their downstream impact on the scheduling objective.
SPO+ is a decision-aware surrogate inspired by the SPO framework that aims to reduce regret, but can be less stable in our scheduling settings where small prediction changes can induce large discrete changes in the resulting schedule.
...
To ground the notation and motivate the design choices, we conclude this section with a running example on outpatient scheduling (Section~\ref{sec:framework-running-example}).
...
(i) pairwise precedence comparisons (e.g., SPT-style ordering
...
\caption{Comparison of SPO+, LTR, and SASS (ours) on \emph{Nonlinear} datasets of size $10000$. SCT error (logarithmic scale) relative to the optimal schedule is shown.}
