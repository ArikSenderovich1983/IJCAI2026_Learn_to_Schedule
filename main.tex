...
\end{equation} Optimizing $L_{\text{regret}}$ is challenging due to non-differentiability and the combinatorial nature of ordering tasks~\cite{elmachtoub2022smart}.
...
\paragraph{Problem 1 (SCT) Generation.} We generate job durations using linear and nonlinear functions. The \emph{linear} model is \(f(x) = \mathbf{xw}\), where \(\mathbf{w}\) is a randomly generated weight vector and \(\mathbf{x}\) consists of features uniformly sampled from \([0,1]\). The \emph{nonlinear} model is \(g(x) = 3x_1^2 + 4.0\sin(2\pi x_2) + 3x_3x_4 + 2x_5^3\), with \(x_1, \dots, x_5\) uniformly sampled. This structure introduces smooth nonlinearities, feature interactions, and periodic components.


\paragraph{Problem 2 (Flow Shop) Generation.} We generate durations for two machines, \(m_1\) and \(m_2\), by incorporating the functions from Problem~1 and adding elements to ensure a balanced probability of assigning jobs to groups \(G_1\) and \(G_2\) (see Section~\ref{sec:flowshop}). In the \emph{linear model}, for each job we sample \(\delta \sim \mathcal{N}(0,1)\) and add it to exactly one machine chosen uniformly at random (leaving the other machine unperturbed).
In the \emph{nonlinear model}, we modify \(g(\mathbf{x})\) by adding or subtracting small linear combinations of input features: \(m_1(\mathbf{x}) = g(\mathbf{x}) + 0.3 x_1 - 0.2 x_3\) and \(m_2(\mathbf{x}) = g(\mathbf{x}) - 0.25 x_2 + 0.4 x_4\). These functions ensure the learning task is nontrivial, requiring the model to capture both local and global feature dependencies while remaining interpretable and reproducible. 

For both problems, an offset is added to guarantee strictly positive durations, and to each of the functions, we add Gaussian noise with mean $0$ and standard deviation \(\eta\) to simulate uncertainty.



\paragraph{Benchmark methods.}
For both problem configurations, we compare against MSE (predict-then-optimize), LTR, and SPO+ (a decision-aware baseline from the SPO line of work).
...
For Problem~1, SASS (ours) achieves the lowest error across all noise levels $\eta$ on \emph{linear} datasets (Table~\ref{tab:linear_dataset_results}).\footnote{For more elaborate results including confidence intervals, see Technical Appendix.} Figure~\ref{fig:nonLinear_problem_1_2} shows the results on \emph{nonlinear} datasets, where SASS remains competitive with the other baselines, which tend to be more stable and exhibit lower standard deviation across runs. MSE performs best at noise levels $0$ and $6$ (Figure~\ref{fig:nonLinear_problem_1_2}), especially at $0$, a typical prediction scenario, but exhibits high variability across runs.
The closest competitor for Problem~1 is LTR, since the optimal solution corresponds to an exact ranking of the jobs by duration.

\begin{table}[t]
\caption{SCT and makespan percentage error with respect to the optimal schedule across noise levels $\eta$ on a \emph{linear} dataset of size $10000$ for Problem~1 and Problem~2 (mean over 10 runs).}
\centering
...
& SASS (ours)  & 0.007 & \textbf{0.042} & 0.132 & 0.128 & \textbf{0.133} & 0.158 \\
...
\end{table}
SPO+, on the other hand, exhibits weak performance both with and without noise, in both \emph{linear} and \emph{nonlinear} settings, as reported in Figure~\ref{fig:nonLinear_problem_1_2}. The weak performance of SPO+ in Problem~1 stems from the definition of the problem itself. SPO+ works best when the optimization objective changes smoothly with respect to the predicted order. In contrast, for the SCT problem, even a small error in the predicted ordering can lead to large deviations in the objective, which makes SPO+ less effective.

Finally, Table~\ref{tab:times_training} reports the training times over Problem~1 for different nonlinear dataset sizes with noise level $5$. As observed, the fastest method is MSE, but it grows roughly with the dataset size since it compares all elements. In contrast, LTR and SASS scale more smoothly thanks to pairwise approximation (Section~\ref{sec:experimental_procedure}).
Regarding SPO+, we observe that its training time is independent of the dataset size but depends on the specific problem instance and its gradient. Additionally, SPO+ requires hyperparameter optimization, taking roughly additional \emph{40 minutes} on average per each dataset and noise level combination, to achieve comparable performance.

Concerning Problem~2 on \emph{linear} datasets, Table~\ref{tab:linear_dataset_results} does not highlight a clear winner. However, SASS consistently ranks first or second, demonstrating robust performance across all noise levels, especially at the highest ones. In contrast, MSE, LTR, and SPO+ show inconsistent behavior, performing best at some noise levels and worst at others.
...
Figure~\ref{fig:nonLinear_problem_1_2} shows a similar pattern for \emph{nonlinear} datasets. SASS remains stable and, although it does not always outperform the baselines, it consistently stays close to the best-performing method. LTR performs well when $\eta = 2$ but very poorly when $\eta = 0$, also exhibiting a large variance in the results. Unlike Problem~1, in this setting LTR does not align with the specific solution of the scheduling problem (the Johnson rule), and its performance therefore deteriorates significantly. SPO+ shows better performance on Problem~2 compared to Problem~1, as mentioned earlier: the smooth convergence of the gradient when minimizing the makespan facilitates the learning phase. However, beyond a noise level of $6$, its performance decreases considerably.


%\begin{figure}[t]
%\centering
%\includegraphics[width=0.5\textwidth]{new_plot/nonLinear_problem1.png}
%\caption{Comparison of MSE, SPO+, LTR, and SASS on \emph{Nonlinear} datasets of size $10000$ on Problem~1. SCT error (logarithmic scale) relative to the optimal schedule is shown.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.47\textwidth]{new_plot/nonLinear_together.png}
\caption{Comparison of SASS and baselines on \emph{Nonlinear} datasets of size $10000$ for Problems~1 and 2, with SCT error shown in log scale (Problem~1) and Makespan percentage error (Problem~2).}
\label{fig:nonLinear_problem_1_2}
\end{figure}
...
%\begin{figure}[t]
%\centering
%\includegraphics[width=0.5\textwidth]{new_plot/nonLinear_2_machine.png}
...
%\label{fig:nonLinear_dataset_2_machine_problem}
%\end{figure}



\section{Case Study: Outpatient Scheduling}
\label{sec:case_study}

We evaluate the proposed SASS surrogate loss on a real-world dataset
from \textit{DayHospital}, an outpatient cancer clinic in the United States. The problem that \textit{DayHospital} faces is to sequence patient 
examinations by physicians with
the objective of minimizing congestion (SCT). Appointment times are known, but exam durations are uncertain and must therefore be predicted.
 The dataset spans the full year (January--December 2021) and contains approximately 201,000 patient pathways derived from real-time location system (RTLS) events.
...
diagnosis, appointment type, and a linkage indicator for subsequent treatments that 
will follow.
To simulate deployment, we trained on the first three weeks of each month and evaluated on the fourth.

We compare SASS against the three baselines using the \emph{relative SCT error} as in the experiments of Problem 1. 
...
Accordingly, we benchmark against SPO+ as the standard decision-aware baseline from the SPO line of work, and we focus on structure-aware surrogates that provide stable, objective-weighted training signals while preserving fast deployment via the known polynomial-time rule.
...
This paper introduced SASS, a structure-aware surrogate learning approach for surrogate-friendly scheduling problems, where the deterministic optimum is given by a known polynomial-time rule that decomposes into critical decisions.
By replacing these discrete decisions with smooth, objective-weighted penalties, SASS yields stable gradients and preserves fast deployment by plugging predictions back into the original scheduling rule at inference time.
We instantiated SASS for single-machine sum of completion times (SPT) and two-machine flow shop makespan minimization (Johnson), and evaluated it on synthetic benchmarks and a real-world outpatient scheduling dataset.
Across experiments, SASS is consistently more stable than an SPO-based surrogate baseline under increasing noise and competitive with MSE and LTR baselines, while achieving the best performance in the majority of real-world months.

Promising future directions include extending SASS beyond pairwise decisions (e.g., incorporating release times, setup times, or multi-resource constraints), developing principled sampling schemes for critical-decision subsets to further scale training, and broadening the surrogate-friendly class to include high-quality heuristics for problems without known optimal polynomial-time rules.
...
In this section, we report additional results and plots that extend the Evaluation section of the paper. We first describe further experimental details regarding the approximation via pairwise comparisons in the SASS approach and the hyperparameter tuning optimization applied to train the SPO+ model. Next, we present additional analysis of the \emph{linear} datasets for Problems 1 and 2.

\paragraph{Experimental setup}

Figure~\ref{fig:pairwise_number} evaluates the performance and training time on a \emph{linear} dataset of size $5000$ with noise level $5$, using different numbers of pairwise comparisons during model training. We consider comparison set sizes ranging from $500$ to $6123250$, which corresponds to all possible pairwise combinations in the training set.
We observe that the training time remains below approximately one minute up to about $900000$ comparisons, after which it increases exponentially.
On the other hand, the performance remains around a 3.2–3.3\% relative error with respect to the optimal SCT, except for the cases with $500$ and $1000$ comparisons, which are insufficient to properly define the model and generalize to the test set.
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.5\textwidth]{new_plot/linear2Machine.png}
\caption{Comparison of SPO+, LTR, and SASS (ours) on \emph{linear} datasets of size $10000$. Makespan error (logarithmic scale) relative to the optimal schedule is shown.}
\label{fig:Linear_dataset_2_machine_problem}
\end{figure}
\begin{table}[t!]
\centering
...
For the SPO+ method, we performed hyperparameter tuning to optimize training performance described in Table~\ref{tab:spo_hyperparams}. The learning rate was searched on a logarithmic scale from $10^{-5}$ to $5 \times 10^{-2}$, the batch size was chosen from $\{5, 10, 20, 30, 50\}$, and the patience for early stopping was varied between 200 and 4000 epochs. This tuning process aimed to balance convergence speed, training stability, and generalization, selecting the combination of hyperparameters that yield the best validation performance.


\paragraph{\emph{Linear} dataset experiments.}
Figure~\ref{fig:Linear_dataset_1_machine_problem} (Problem~1) and Figure~\ref{fig:Linear_dataset_2_machine_problem} (Problem~2) visualize the results from Table~\ref{tab:linear_dataset_results}. Each curve reports the mean over $10$ runs and the shaded region indicates the corresponding confidence interval.

For Problem 1, MSE exhibits substantially higher variability across runs, whereas LTR and SASS degrade smoothly as noise increases. Across noise levels, SASS achieves the lowest (or near-lowest) SCT error and remains stable.
For Problem 2, no single method dominates across all noise levels. In particular, MSE and SPO+ can be either best or worst depending on $\eta$ and exhibit higher variance. SASS is not always the top performer, but it is consistently competitive and comparatively robust to noise.
