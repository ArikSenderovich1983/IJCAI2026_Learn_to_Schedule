...
encouraging a fixed sign), we also define a \emph{ground-truth decision label}
$y_d(\theta^\star)\in\{-1,+1\}$ indicating the correct outcome for decision $d$
under the true parameters:
\[
y_d(\theta^\star)=+1 \ \text{if the correct outcome matches } s_d>0,\qquad
y_d(\theta^\star)=-1 \ \text{otherwise.}
\]
We then use the smooth misclassification indicator
\[
\sigma\!\left( -\frac{y_d(\theta^\star)\, s_d(\omega;x)}{\lambda} \right),
\]
...
\phi_d(\theta^\star)\,
\sigma\!\left(
\frac{-y_d(\theta^\star)\, s_d(\omega;x)}{\lambda}
\right),
\end{equation}
...
$\hat{y}_i = f_\omega(x_i)$ from a dataset
$\mathcal{D} = \{(x_j, y_j)\}_{j=1}^N$.

\subsection{Step 2: Deterministic Polynomial-Time Counterpart}
\label{sec:sct-step2}
...
L_{\text{regret}} = \sum_{\substack{y_i < y_j}} R_{i,j} \cdot \max(0, \text{sign}(f_\omega(x_i) - f_\omega(x_j))).
\end{equation}
\end{problem}
...
\subsection{Step 3: A Sigmoid Surrogate for SCT}
\label{sec:methodology}

The critical decisions for SPT are pairwise precedence comparisons.
For a pair $(i,j)$, define the decision score
\(
s_{i,j}(\omega;x)= f_\omega(x_i)-f_\omega(x_j)
\),
whose sign encodes the model's preference.
When $y_i<y_j$, the correct order is $i$ before $j$, and a misordering occurs
...
By minimizing the empirical sigmoid loss, the learned model \(f_\omega\) is encouraged to satisfy \(f_\omega(x_j) > f_\omega(x_i)\) whenever \(y_j > y_i\).
Formally:
\[
\omega^\star
\;=\;
...
\subsection{Step 4: Train and Infer with SPT}
\label{sec:sct-step4}

At inference time, given a new instance with features $\{x_i\}_{i=1}^n$, we
compute predicted durations $\hat{y}_i=f_{\omega^\star}(x_i)$ and return the
SPT schedule obtained by sorting tasks by $\hat{y}_i$ in ascending order.
...
\subsection{Step 1: Identify Unknown Parameters}
\label{sec:flowshop-step1}

In the $F2\|C_{\max}$ setting, the unknown parameters for each job $i$ are its
two processing times $(p_{i1},p_{i2})$ on machines $1$ and $2$.

\subsection{Step 2: Deterministic Polynomial-Time Counterpart}
\label{sec:flowshop-step2}
...
\[
(\hat{d}_i, \hat{s}_i) = f_\omega(x_i),
\]
...
\subsection{Step 3: A Sigmoid-Based Surrogate for Johnson's Rule}
\label{sec:flowshop-step3}
...
\subsection{Step 4: Train and Infer with Johnson's Rule}
\label{sec:flowshop-step4}
...
These visits consist of a sequence of activities, which may include a blood draw, an examination, and chemotherapy infusion. 
