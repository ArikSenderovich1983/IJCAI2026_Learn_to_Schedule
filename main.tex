...
\noindent\textbf{Step 2: Deterministic Polynomial-Time Counterpart and Critical Decisions.}
...
Following Section~\ref{sec:framework}, the critical decision set for SPT is the collection of pairwise precedence comparisons,
\(
\mathcal{K} := \{d=(i,j): i<j\},
\)
where each $d$ encodes whether job $i$ should precede job $j$.

\paragraph{Predict-then-optimize baseline (MSE).}
As a baseline, we consider predict-then-optimize~\cite{Mandi2020}, where $f_\omega$ is trained by minimizing empirical mean squared error (MSE):
\begin{equation}
...
We train by minimizing $L_{\text{sigmoid}}$: correctly ordered pairs contribute near $0$, while misordered pairs are penalized proportionally to their SCT impact $R_{i,j}$.
\[
\omega^\star = \arg\min_{\omega} L_{\text{sigmoid}}(\omega).
\]
