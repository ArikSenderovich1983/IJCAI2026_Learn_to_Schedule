...
Figure~\ref{fig:Linear_dataset_1_machine_problem} (Problem~1) and Figure~\ref{fig:Linear_dataset_2_machine_problem} (Problem~2) visualize the linear-dataset results from Table~\ref{tab:linear_dataset_results}. Each curve reports the mean over $10$ runs and the shaded region indicates the corresponding confidence interval.

\textbf{Problem~1 (SCT).} MSE exhibits substantially higher variability across runs, whereas LTR and SASS degrade smoothly as noise increases. Across noise levels, SASS achieves the lowest (or near-lowest) SCT error and remains stable.

\textbf{Problem~2 (Makespan).} No single method dominates across all noise levels. In particular, MSE and SPO+ can be either best or worst depending on $\eta$ and exhibit higher variance. SASS is not always the top performer, but it is consistently competitive and comparatively robust to noise.
