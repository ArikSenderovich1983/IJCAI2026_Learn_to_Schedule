...
In particular, comparison-based rules (sorting, thresholding, group assignment) can change the schedule abruptly: a small perturbation in predictions can flip a few critical decisions and yield a different permutation, producing a piecewise-constant objective with many discontinuity boundaries and unstable training signals.
...
Concern Problem~2, SASS outperforms the other method in precense of a relevant noise level on \emph{linear} datasets as shown in Table~\ref{tab:linear_dataset_results}. 
...
& SASS  & 0.007 & \textbf{0.042} & 0.132 & \textbf{0.128} & \textbf{0.133} & \textbf{0.158} \\
& SPO+  & 0.012    &       &       &       &       &       \\
\bottomrule
\end{tabular}}
