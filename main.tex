...
\paragraph{Problem 2 (Flow Shop) Generation.} We generate durations for two machines, \(m_1\) and \(m_2\), by incorporating the functions from Problem~1 and adding elements to ensure a balanced probability of assigning jobs to groups \(G_1\) and \(G_2\) (see Section~\ref{sec:flowshop}). In the \emph{linear model}, we introduce \(\delta\) as a random variable sampled from a normal distribution in \([0,1]\) and set the duration for either machine as \(m_i(\mathbf{x}) = \mathbf{xw} + \delta\) for \(i=1,2\).
...
Starting from Problem~1, SASS outperforms the baselines across all noise levels $\eta$ on \emph{linear} datasets, as shown in Table~\ref{tab:linear_dataset_results}.\footnote{For more elaborate results including confidence intervals, see Technical Appendix.} Figure~\ref{fig:nonLinear_problem_1_2} shows the results on \emph{nonlinear} datasets, where SASS remains competitive with the other baselines, which tend to be more stable and exhibit lower standard deviation across runs. MSE performs best at noise levels $0$ and $6$ (Figure~\ref{fig:nonLinear_problem_1_2}), especially at $0$, a typical prediction scenario, but exhibits high variability across runs.
...
& SASS  & 0.007 & \textbf{0.042} & 0.132 & 0.128 & \textbf{0.133} & 0.158 \\
...
SASS & 28.64s & 33.26s & 41.57s & 52.59s \\
...
will follow.
To simulate deployment, we trained on the first three weeks of each month and evaluating on the fourth.
...
Figure~\ref{fig:Linear_dataset_1_machine_problem} and Figure~\ref{fig:Linear_dataset_2_machine_problem} represent the results reported in Table~\ref{tab:linear_dataset_results} on \emph{linear} datasets for Problems 1 and 2, respectively, also showing the confidence intervals around the mean computed over $10$ runs for each method and all noise levels.
Figure~\ref{fig:Linear_dataset_1_machine_problem} highlights the large variance of MSE, while LTR and SASS show a roughly linear degradation as noise increases. SASS consistently achieves a lower SCT error.
Concerning Problem~2, Figure~\ref{fig:Linear_dataset_2_machine_problem}, as highlighted in Table~\ref{tab:linear_dataset_results}, shows that no method consistently outperforms the others across all noise levels. In particular, MSE and SPO+ lack robustness and can be the best or worst methods depending on the noise level, also exhibiting large variance across multiple runs. However, SASS, while not always the best, demonstrates consistent performance regardless of the presence of noise.
