...
\paragraph{Problem 2 (Flow Shop) Generation.} We generate durations for two machines, \(m_1\) and \(m_2\), by incorporating the functions from Problem~1 and adding elements to ensure a balanced probability of assigning jobs to groups \(G_1\) and \(G_2\) (see Section~\ref{sec:flowshop}). In the \emph{linear model}, we introduce \(\delta\) as a random variable sampled from a standard normal distribution, and add $\delta$ to one of the machines at random, \(m_i(\mathbf{x}) = \mathbf{xw} + \delta\) for either $i=1$ or $i=2$.
