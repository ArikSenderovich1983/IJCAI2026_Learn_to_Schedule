...
\title{SASS: Structure-Aware Surrogate Scheduling}
...
On synthetic benchmarks, SASS is substantially more stable than SPO+ under increasing noise and is competitive with predict-then-optimize (MSE) baselines.
...
\section{SASS: Surrogate-Friendly Predict-and-Schedule}
\label{sec:framework}
...
For MSE, LTR and our method we use the same two-hidden-layer neural network architecture, number of iterations and, finally, number of pair comparisons, to properly evaluate which of them converges first to a low error. Instead for SPO+, we use a specific hyperparameter optimisation, especially to define the batch size\footnote{In the repository \url{https://github.com/francescameneghello/....}, the implementation details are reported.}
