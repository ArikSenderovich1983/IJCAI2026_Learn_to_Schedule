...
Following Section~\ref{sec:framework}, we make the critical decision set explicit:
\(
\mathcal{K}=\mathcal{K}_{\mathrm{group}}\cup\mathcal{K}_{\mathrm{order}},
\)
where $\mathcal{K}_{\mathrm{group}}=\{d=i: i\in[n]\}$ encodes group membership decisions and $\mathcal{K}_{\mathrm{order}}=\{d=(i,j): i<j,\ i,j \in G_1 \text{ or } i,j \in G_2\}$ encodes within-group precedence comparisons.
