...
\begin{abstract}
Task scheduling is a fundamental challenge in artificial intelligence and operations research, with applications in manufacturing, logistics, cloud computing, and healthcare.
In settings with uncertainty and historical data, smart predict-and-optimize (SPO) methods offer a promising data-driven approach by minimizing empirical regret---the gap between the learned and optimal solutions on training data.
However, applying SPO to even simple decision problems is often hindered by the non-differentiability of common objective functions and by the discrete nature of optimal policies.
We address these limitations by proposing a surrogate-based framework that constructs smooth learning objectives from deterministic, polynomial-time decision rules, and by instantiating this framework on two classical scheduling problems.
Our main instantiation introduces a sigmoid-based surrogate loss that approximates the regret in minimizing the sum of completion times on a single machine, while a second instantiation targets the two-machine flow shop scheduling problem via a surrogate aligned with Johnson's rule.
Experiments on synthetic benchmarks and a real-world healthcare scheduling dataset show that our approach consistently outperforms state-of-the-art SPO and learn-to-rank methods across the vast majority of problem instances.
\end{abstract}

\section{Introduction}
\label{sec:introduction}

Scheduling involves allocating limited resources to tasks over time to optimize objectives such as minimizing the sum of completion times (SCT) or maximizing throughput~\cite{baker2018principles,pinedo2022scheduling}.
...
Classical approaches typically assume that task parameters (e.g., processing times) are known and deterministic, enabling the use of efficient algorithms and priority rules.
In reality, however, these parameters are uncertain and must be inferred from historical data.
This mismatch between deterministic models and stochastic environments motivates the development of data-driven decision-making methods that explicitly account for uncertainty.
...
Second, even when differentiable surrogates such as SPO+ are available, they may be poorly aligned with the structure of specific combinatorial problems and can exhibit unstable training behaviour.

In this paper, we develop a surrogate-based framework that addresses these challenges for a broad class of decision problems whose deterministic variants are solvable in polynomial time.
The key idea is to exploit the structure of known optimal algorithms---for example, sorting rules in scheduling or Johnson's rule in flow shops---to construct smooth surrogate losses that approximate regret.
These surrogates are built from local comparisons (e.g., pairwise orderings or group assignments) that mirror the operations of the underlying algorithm, and are relaxed via sigmoidal functions to enable end-to-end gradient-based training.

We then specialize this general framework to scheduling under uncertainty.
...
We further show how the same principles extend to a two-machine flow shop with makespan minimization, where the deterministic optimal policy is given by Johnson's rule.
...
On synthetic instances of the SCT problem, our surrogate consistently outperforms SPO+ and logistic rank-loss baselines, especially under high noise and large duration variability.
...
\begin{enumerate}
    \item We propose a general surrogate-based framework for predict-and-optimize, applicable to decision problems whose deterministic variants admit polynomial-time optimal algorithms.
    \item We instantiate this framework on two classical scheduling problems: (i) single-machine SCT minimization and (ii) two-machine flow shop makespan minimization, deriving problem-specific sigmoid-based surrogates.
    \item We provide theoretical guarantees, including differentiability and convergence of the surrogate to regret in the SCT problem, and show how algorithmic structure (e.g., Johnson's rule) guides surrogate design in the flow shop setting.
    \item We empirically evaluate our approach on synthetic and real-world scheduling datasets, demonstrating consistent improvements over state-of-the-art SPO and learn-to-rank methods.
\end{enumerate}
...
deterministic variant is polynomial-time solvable.
In this section, we outline a
general framework for integrating prediction and optimization through
surrogate losses derived from deterministic optimal policies.
This framework
...
\mathbb{R}^p$.
The decision problem depends on unknown parameters
...
transition costs.
A feasible decision is represented by $\pi \in \Pi$, where
$\Pi$ denotes the set of allowable schedules or action plans.
The true objective
...
its contribution to the optimization problem.
For scheduling, these may be
processing times or resource requirements.
Let a dataset
...
In this paper, we demonstrate two scheduling problems
...
Scheduling problems are a natural fit for the proposed framework.
When task durations or other parameters are known, many classical scheduling problems admit simple optimal policies based on sorting or partitioning rules.
For example, minimizing SCT on a single machine is solved by sorting jobs in non-decreasing order of processing times, while the two-machine flow shop with makespan minimization can be solved optimally via Johnson's rule.

Under uncertainty, task parameters must be predicted from features.
Naively learning to minimize mean squared error (MSE) on durations ignores the downstream scheduling objective and treats all prediction errors equally, even though only errors that alter the induced schedule affect SCT or makespan.
Our framework instead constructs surrogate losses that mirror the structure of the deterministic optimal policy: pairwise comparisons in single-machine scheduling and group assignments plus within-group orderings in the flow shop.
These surrogates weight prediction errors by their impact on the objective, yielding learning algorithms that are explicitly aligned with scheduling regret.

In the remainder of the paper, we instantiate this framework on two canonical problems:
single-machine SCT minimization (Problem~1) and two-machine flow shop makespan minimization (Problem~2).
For each, we define the underlying scheduling problem under uncertainty, derive a sigmoid-based surrogate loss tailored to its deterministic optimal policy, and empirically evaluate the resulting learning algorithms.

\section{Problem 1: Single-Machine Sum of Completion Times}
\label{sec:problem1}

We first examine smart predict-and-optimize methods applied to single-machine scheduling with the SCT objective.
We begin by defining the basic scheduling problem under consideration, namely the single-machine sum of completion times minimization problem.
We then describe the task uncertainty model, formulate the learning problem, and introduce a surrogate loss that approximates regret while remaining differentiable.

\subsection{Scheduling Problem Under Uncertainty}
...
Adopting a supervised learning framework, let $x$ denote a $p$-dimensional feature vector describing a task.
For $n$ independent tasks with unknown durations $y_i$ ($i=1,\ldots,n$), we assume the following model~\cite{hastie2009elements}:
...
A dataset \(\mathcal{D} = \{(x_j, y_j)\}_{j=1}^{N}\) contains historical feature-duration pairs, where typically \(N \gg n\).
The goal is to leverage this historical data to learn a predictor \(\hat{f}(x)\) to optimize scheduling decisions.

A straightforward approach is the predict-then-optimize paradigm~\cite{Mandi2020}, in which \(\hat{f}\) is learned by minimizing empirical mean squared error (MSE):
\begin{equation}
\hat{f} = \arg\min_{f \in \mathcal{F}} \sum_{i=1}^{N} \bigl(f(x_i) - y_i\bigr)^2,
\end{equation}
where \(\mathcal{F}\) denotes the function class (e.g., linear models, neural networks).
The resulting model predicts durations, and the optimization phase solves the scheduling problem using these predictions.
...
If \(\hat{f}(x_i) > \hat{f}(x_j)\), the tasks are misordered, increasing the SCT.
...
Yet, directly optimizing regret is challenging due to the non-differentiability of the objective and the combinatorial nature of ordering tasks~\cite{elmachtoub2022smart}.
To address this, we now propose a differentiable surrogate loss that approximates pairwise misorderings, facilitating end-to-end learning for smart predict-and-schedule.

\subsection{Sigmoid Surrogate for SCT}
\label{sec:methodology}

We next present the main contribution for Problem~1, namely a surrogate-based loss function that approximates regret in the SCT problem while remaining differentiable and suitable for gradient-based optimization.

\subsubsection{Sigmoid loss for SPO}
...
By minimizing the empirical sigmoid loss, the learned model \(\hat{f}\) is encouraged to satisfy \(\hat{f}(x_j) > \hat{f}(x_i)\) whenever \(y_j > y_i\).
Formally:
\[
\hat{f}
\;=\;
\arg\min_{f\in\mathcal{F}}
  L_{\text{sigmoid}}(f),
\]
where \(\mathcal{F}\) is the space of candidate functions, such as neural networks.
Thus, training adjusts \(\hat{f}\) to minimize the number and severity of misordered pairs, aligning predictions with true task durations.
When \(\hat{f}\) preserves correct orderings, the sigmoid term remains near zero, driving \(L_{\text{sigmoid}}\) downward.
...
\section{Problem 2: Two-Machine Flow Shop Scheduling}
\label{sec:flowshop}
...
\emph{the structural decisions} induced by Johnson’s rule.
To reflect this, each
...
As with the SCT surrogate, letting $\lambda \to 0$
...
\btext{
For two jobs $i,j$, the cost of the swap is given by:

\begin{equation}
T_{i,j} = p_{1,i} + max(p_{2,i},p_{1,j}) + p_{2,j} 
\end{equation}
\begin{equation}
    T_{j,i} = p_{1,j} + max(p_{2,j},p_{1,i}) + p_{2,i}
\end{equation}
\begin{equation}
    \Delta = T_{j,i} - T_{i,j}
\end{equation}
...
:=
\sum_{\substack{i,j \in G_1 \\ p_{i1} < p_{j1}}}
\Delta\, \sigma\Big( \frac{(\hat{f}(p_{i1}) - \hat{f}(p_{j1}))}{\lambda} \Big)
+
\sum_{\substack{i,j \in G_2 \\ p_{i2} > p_{j2}}}
\Delta\, \sigma\Big( \frac{(\hat{f}(p_{i2}) - \hat{f}(p_{j2}))}{\lambda} \Big)
$}
}
...
Gaussian noise with standard deviation \(\sigma \in [0.1, 5]\) was added to simulate uncertainty.
Task duration variability was adjusted through the \texttt{scale\_factor}.
...
The second is the \emph{logistic rank loss}, a learn-to-rank (LTR) surrogate loss \cite{fu2024efficient} that penalizes incorrect pairwise orderings using logistic regression.
...
We start by describing the case, followed by the experimental setup and procedure, and lastly, we provide the results 
and discuss their implications. 

\subsection{Case Description: Scheduling in DayHospital}

At \textit{DayHospital}, patients typically arrive at the clinic in the morning and complete their visits in the afternoon.
These visits consist of a sequence of activities, which may include:
\begin{enumerate}
...
\end{enumerate}
In this work, we focus on the \emph{examination stage} of the process.
In our model, \textit{doctors are treated as single-server machines}, and \textit{patients are modeled as jobs} in a scheduling system.
Each patient is assigned a pre-scheduled appointment time for the exam, which we treat as the known arrival time to the system.
However, the \emph{duration of each examination is uncertain} and not known in advance.

This setting reflects a typical outpatient environment in which variability and uncertainty in service times complicate the planning and sequencing of activities.
While some patients may skip the examination altogether, it remains a central point in the care flow for many.
Understanding and addressing the uncertainty in examination durations is therefore essential for optimizing provider schedules and reducing patient delays throughout the system.

\subsection{Experimental Setup}

\paragraph{Dataset.} 
We used two months of operational data from \textit{DayHospital}, specifically January and November 2021.
Two primary sources of data were available for analysis:
\begin{itemize}
...
\end{itemize}
The dataset includes approximately 201,000 patient pathways, all of which involve a visit to the blood draw station.
For each \textit{visible patient} at this station, we observe three key timestamps: the time of arrival, the time at which service begins, and the time at which service is completed. 

Each record represents a scheduled medical appointment, characterized by several features.
Following preprocessing, we filtered the dataset to focus specifically on \textit{Exam} appointments.
The objective was to learn a model that predicts the actual appointment durations \( y_i \) from the available features \( x_i \), in a way that enables task sequencing to minimize SCT.
...
To better reflect operational deployment, we organized the data by calendar months.
For each month, the models were trained on the first three weeks of appointments and evaluated on the appointments from the fourth week.
This setup simulates a realistic rolling horizon setting where models must generalize to near-future scheduling scenarios.
...
\begin{itemize}
    ...
    \item \emph{Mean Squared Error (MSE)}: A standard regression loss that minimizes squared differences between predicted and true durations, without considering the downstream scheduling objective.
    This is in essence a predict-then-optimize approach~\cite{shahabikargar2014predicting}.
...
This reflects the scheduling regret incurred by each model and aligns with the evaluation used in the synthetic setting.

\subsection{Experimental Procedure}

The models were implemented as feedforward neural networks consisting of two hidden layers with ReLU activations and a Softplus output layer.
For consistency across experiments, all models were trained using the Adam optimizer with a learning rate of \(5 \times 10^{-3}\) and weight decay of \(1 \times 10^{-4}\).
Early stopping was applied based on convergence of the training loss to prevent overfitting.

Each method was trained for up to 2000 iterations, with the relative SCT error evaluated every 50 steps.
The primary evaluation metric was the relative SCT error on the test set, computed by sequencing tasks using the model-predicted durations \(\hat{y}_i\) and comparing the resulting SCT to that of the optimal ordering under true durations \(y_i\).

The SPO+ implementation utilized a mini-batch version of the surrogate loss, which incorporates a differentiable linear program to minimize cumulative completion time based on the predicted durations.
This formulation enabled gradient-based optimization.
All methods were evaluated using the same training--test splits and comparable model capacity to ensure a fair comparison.

To assess robustness over time, we repeated this experiment using data from multiple months in the year 2021.
...
Figure~\ref{fig:real-data-jan} shows the relative SCT error over training iterations for each method on this dataset.
The y-axis represents the SCT error relative to the optimal ordering, and the x-axis indicates the training steps.
...
The results demonstrate that the sigmoid-based regret surrogate consistently achieves the lowest SCT error, outperforming all baselines throughout training.
Rank Loss performs competitively early on but plateaus at a higher error.
MSE converges more slowly and ultimately underperforms, confirming that minimizing prediction error alone does not lead to better scheduling.
SPO+ converges but stabilizes at a higher SCT error, indicating sensitivity to model dynamics and optimization noise despite accounting for task ordering.
