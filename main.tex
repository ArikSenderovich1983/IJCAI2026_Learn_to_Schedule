...
The resulting objective value under the true parameters is
\[
C(\pi;\theta^\star),
\]
where $C$ may represent, for example, sum of completion times, makespan, or a
waiting-time objective.

The learning task is to construct a predictor
$\hat{\theta}=f_\omega(x)$ (with parameters $\omega$) and then apply a
deterministic scheduling rule to $\hat{\theta}$ to obtain a schedule
$\pi(\hat{\theta})$.
...
\[
\sigma\!\left( \frac{s_d(\omega;x)}{\lambda} \right),
\]
...
w_d(\theta^\star)\,
\sigma\!\left(
\frac{s_d(\omega;x)}{\lambda}
\right),
\end{equation}
where $w_d(\theta^\star)\ge 0$ measures the impact of deciding $d$ incorrectly
...
Patients $i=1,\dots,n$ arrive with known appointment/arrival times $a_i$ and
uncertain exam durations.
Depending on the operational objective, a deterministic policy might implement
FIFO (serve in order of $a_i$), an EDD-like rule (serve those with earlier due
times first), or an SPT-like rule (serve those with shorter predicted exams
...
The surrogate~\eqref{eq:general-surrogate} penalizes incorrect or ambiguous
decisions through $\sigma(s_d/\lambda)$ and uses $w_d(\theta^\star)$ to reflect
...
critical decisions $\mathcal{D}$ and (ii) the weights $w_d$.
